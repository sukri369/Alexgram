...
public class PluginsActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public View createView(Context context) {
        ...
        actionBar.setTitle(LocaleController.getString("Plugins", R.string.Plugins));
    ...
    private void pickPluginFile() {
        ...
        try {
            ...
            getParentActivity().startActivityForResult(
                    Intent.createChooser(intent, LocaleController.getString("ChoosePluginFile", R.string.ChoosePluginFile)), REQUEST_PICK);
        } catch (Throwable t) {
            FileLog.e(t);
            BulletinFactory.of(this).createErrorBulletin(LocaleController.getString("CouldNotOpenFileSelector", R.string.CouldNotOpenFileSelector)).show();
        }
    }
    ...
    private void importUri(Uri uri) {
        Utilities.globalQueue.postRunnable(() -> {
            ...
            AndroidUtilities.runOnUIThread(() -> {
                if (result == null) {
                    BulletinFactory.of(this).createErrorBulletin(LocaleController.getString("NotValidPluginFile", R.string.NotValidPluginFile)).show();
                } else {
                    updateRows();
                    if (adapter != null) {
                        adapter.notifyDataSetChanged();
                    }
                    BulletinFactory.of(this).createSuccessBulletin(LocaleController.formatString("PluginInstalled", R.string.PluginInstalled, result.displayName())).show();
                }
            });
        });
    }
    ...
    public static boolean offerInstall(android.app.Activity activity, File file) {
        ...
        if (isUpdate) {
            ...
            sb.append("\n\n").append(LocaleController.formatString("AlreadyInstalledVersion", R.string.AlreadyInstalledVersion, cur, next));
        }
        ...
        if (!TextUtils.isEmpty(meta.minVersion) && PluginsController.compareVersions(
            ...
            sb.append("\n\n⚠ ").append(LocaleController.formatString("RequiresVersion", R.string.RequiresVersion, meta.minVersion));
        }
        org.telegram.ui.ActionBar.AlertDialog.Builder builder =
                new org.telegram.ui.ActionBar.AlertDialog.Builder(activity);
        builder.setTitle(isUpdate ? LocaleController.getString("UpdatePlugin", R.string.UpdatePlugin) : LocaleController.getString("InstallPlugin", R.string.InstallPlugin));
        builder.setMessage(sb.toString());
        builder.setPositiveButton(isUpdate ? LocaleController.getString("Update", R.string.Update) : LocaleController.getString("Install", R.string.Install), (dialog, which) -> {
            dialog.dismiss();
            ...
            if (installed != null && last != null) {
                ...
                BulletinFactory.global().createSuccessBulletin(
                        LocaleController.formatString("PluginInstalled", R.string.PluginInstalled, installed.displayName())).show();
            } else {
                BulletinFactory.global().createErrorBulletin(LocaleController.getString("FailedToInstallPlugin", R.string.FailedToInstallPlugin)).show();
            }
        });
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            switch (getItemViewType(position)) {
                case VIEW_TYPE_HEADER:
                    ((HeaderCell) holder.itemView).setText(LocaleController.getString("InstalledPlugins", R.string.InstalledPlugins));
                    break;
                case VIEW_TYPE_INFO:
                    String hint = plugins.isEmpty()
                            ? LocaleController.getString("NoPluginsHint", R.string.NoPluginsHint)
                            : LocaleController.getString("PluginTapHint", R.string.PluginTapHint);
                ...
                default:
                    ...
                    String status;
                    if (info.error != null) {
                        status = LocaleController.getString("Error", R.string.Error);
                    } else if (!info.enabled) {
                        status = LocaleController.getString("Disabled", R.string.Disabled);
                    } else {
                        status = info.version != null ? "v" + info.version : LocaleController.getString("Enabled", R.string.Enabled);
                    }
