    ...
    FactorAnimator.Target
{
    ...
    private Rect sendRect = new Rect();
...
//            }
        ...
        messageEditTextContainer.addView(emojiButton, LayoutHelper.createFrame(DEFAULT_HEIGHT, DEFAULT_HEIGHT, (iosStyle ? Gravity.RIGHT : Gravity.LEFT) | Gravity.BOTTOM, iosStyle ? 0 : 2, 0, iosStyle ? 0 : 0, 0));
                    ...
                    if (alpha < 0.45) {
                        ...
                        if (recordAudioVideoRunnableStarted) {
                            ...
                            if (hasRecordVideo && isInVideoMode()) {
                                ...
                                if (!NekoConfig.confirmAVMessage.Bool() && recordingAudioVideo && isInScheduleMode()) {
                                    AlertsCreator.createScheduleDatePickerDialog(parentActivity, parentFragment.getDialogId(), (notify, scheduleDate, scheduleRepeatPeriod) -> MediaController.getInstance().stopRecording(1, notify, scheduleDate, voiceOnce, 0), () -> MediaController.getInstance().stopRecording(0, false, 0, voiceOnce, 0), resourcesProvider);
                                }
                                delegate.needStartRecordAudio(0);
                                MediaController.getInstance().stopRecording(NekoConfig.confirmAVMessage.Bool() ? 2 : isInScheduleMode() ? 3 : 1, true, 0, voiceOnce, 0);
                            }
                            if (!NekoConfig.confirmAVMessage.Bool()) {
                                recordingAudioVideo = false;
                                messageTransitionIsRunning = false;
    ...
    private void createSenderSelectView() {
        ...
        senderSelectView.setOnClickListener(v -> {
            ...
            if (delegate.getSendAsPeers() != null) {
                ...
                senderSelectPopupWindow.getContentView().measure(MeasureSpec.makeMeasureSpec(dp(1000), MeasureSpec.AT_MOST), MeasureSpec.makeMeasureSpec(dp(1000), MeasureSpec.AT_MOST));
...
//            }
    ...
    public void drawBackground(Canvas canvas, boolean withComposeShadowDrawable) {
        ...
        if (withComposeShadowDrawable && !iosStyle) {
    ...
    private void onMenuClick(View view) {
        ...
        if (!isInInput) {
            ...
            // Send video as Record (round video message)
            cell = new ActionBarMenuSubItem(getContext(), false, true);
                ...
                if (checkMenuPermissions(true)) {
                    if (parentFragment instanceof ChatActivity) {
                        try {
                            ...
                            android.content.Intent videoPickerIntent = new android.content.Intent(android.content.Intent.ACTION_PICK);
                            videoPickerIntent.setType("video/*");
                            // Use request code 71 for round video picking
                            parentFragment.startActivityForResult(videoPickerIntent, 71);
                        } catch (Exception e) {
        ...
        if (isStories || (messageEditText == null || TextUtils.isEmpty(messageEditText.getText())) && parentFragment != null && parentFragment.messagePreviewParams != null && parentFragment.messagePreviewParams.forwardMessages != null && parentFragment.messagePreviewParams.forwardMessages.messages != null && !parentFragment.messagePreviewParams.forwardMessages.messages.isEmpty()) {
            ...
            if (sendPopupLayout == null) {
                ...
                if (messageEditText != null && messageEditText.getText().length() > 0) {
                    ...
                    preSendTranslateButton.setOnLongClickListener(view1 -> {
                        if (sendPopupWindow != null && sendPopupWindow.isShowing()) {
                            sendPopupWindow.dismiss();
                        }
                        Translator.showTargetLangSelect(view, true, (locale) -> {
...
//        }
    ...
    public void updateFieldHint(boolean animated) {
        ...
        if (parentFragment != null && parentFragment.getChatMode() == ChatActivity.MODE_QUICK_REPLIES) {
            ...
            if (anonymously) {
                messageEditText.setHintText(getString(R.string.SendAnonymously));
            } else {
                ...
                if (user != null && user.bot_forum_view && !user.bot_forum_can_manage_topics && parentFragment != null && !parentFragment.isTopic) {
                    ...
                    if (parentFragment.isReplyChatComment()) {
                        ...
                        messageEditText.setHintText(getString(R.string.Reply));
                    }
                } else if (isChannel) {
                    if (silent) {
                        messageEditText.setHintText(getString(R.string.ChannelSilentBroadcast), animated);
                    } else {
    ...
    public void setEditingMessageObject(MessageObject messageObject, MessageObject.GroupedMessages groupedMessages, boolean caption) {
        ...
        if (editingMessageObject != null) {
            ...
            if (editingMessageObject.needResendWhenEdit() && paidMessagesPrice > 0) {
                ...
                doneButton.setLayoutParams(LayoutHelper.createFrame(iosStyle ? 48 : DEFAULT_HEIGHT, iosStyle ? 48 : DEFAULT_HEIGHT, (iosStyle ? Gravity.LEFT : Gravity.RIGHT) | Gravity.BOTTOM, iosStyle ? 8 : 0, 0, iosStyle ? 0 : 0, 0));
...
//        }
    ...
    public void setBotsCount(int count, boolean hasCommands, boolean hasQuickReplies, boolean animated) {
        ...
        if (hasBotCommands != hasCommands || this.hasQuickReplies != hasQuickReplies) {
            ...
            if (!hasText()) updateBotButton(animated);
        }
    }
    ...
    public void setButtons(MessageObject messageObject, boolean openKeyboard) {
        ...
        if (botKeyboardView == null) {
            ...
            botKeyboardView.setDelegate(button -> {
                ...
                if (delegate != null) {
                    delegate.beforeMessageSend(null, true, 0, 0);
                }
    ...
    protected void createEmojiView() {
        ...
        emojiView.setDelegate(new EmojiView.EmojiViewDelegate() {
            ...
            public void onGifSelectedForAddCaption(View view, Object gif, String query, Object parent, boolean notify, int scheduleDate, int scheduleRepeatPeriod) {
                PhotoViewer.getInstance().setParentActivity(parentFragment != null ? parentFragment : (BaseFragment) parentActivity, resourcesProvider);
                File file = null;
            ...
            public void onGifSelected(View view, Object gif, String query, Object parent, boolean notify, int scheduleDate, int scheduleRepeatPeriod, MediaController.PhotoEntry entry) {
                if (replyingQuote != null && parentFragment != null && replyingQuote.outdated) {
                    parentFragment.showQuoteMessageUpdate();
                    return;
                }
                if (isInScheduleMode() && scheduleDate == 0) {
                    AlertsCreator.createScheduleDatePickerDialog(parentActivity, parentFragment.getDialogId(), (n, s, r) -> onGifSelected(view, gif, query, parent, n, s, r, entry), resourcesProvider);
                } else {
                    if (slowModeTimer > 0 && !isInScheduleMode()) {
                        if (delegate != null) {
                            delegate.onUpdateSlowModeButton(view != null ? view : slowModeButton, true, slowModeButton.getText());
                        ...
                        Runnable runnable = () -> {
                            ...
                            if (gif instanceof TLRPC.Document) {
                                ...
                                if (NaConfig.INSTANCE.getAutoInsertGIFCaption().Bool()) {
                                    ...
                                    SendMessagesHelper.getInstance(currentAccount).sendSticker(document, query, dialog_id, replyingMessageObject, getThreadMessage(), storyItem, replyingQuote, videoEditedInfo, notify, scheduleDate, scheduleRepeatPeriod, false, parent, parentFragment != null ? parentFragment.quickReplyShortcut : null, parentFragment != null ? parentFragment.getQuickReplyId() : 0, stars, getSendMonoForumPeerId(), getSendMessageSuggestionParams(), entry != null ? entry.caption : null, entry != null ? entry.entities : null);
                                }
...
//                }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        } else if (!iosStyle && senderSelectView != null && senderSelectView.getVisibility() == View.VISIBLE) {
...
//            }
    ...
    public void setLiveComment(boolean isLiveComment, boolean isAdmin) {
        ...
        attachButton.setVisibility((isLiveComment || (iosStyle && editingMessageObject != null)) ? View.GONE : View.VISIBLE);
        if (isLiveComment) {
